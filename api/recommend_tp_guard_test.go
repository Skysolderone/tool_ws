...
func TestApplyTakeProfitGuardFallback(t *testing.T) {
	...
	if math.Abs(tp-103.0) > 1e-9 {
		t.Fatalf("unexpected fallback tp: got=%.4f want=103.0", tp)
	}
	if len(reasons) == 0 {
...
func TestApplyTakeProfitGuardRiskReward(t *testing.T) {
	// rr = 1.3 / 1.0 = 1.3 < 1.6，应该被提升到 1.6
	tp, reasons := applyTakeProfitGuard("LONG", 100, 99, 101.30, nil, []string{"base"})
	if math.Abs(tp-101.60) > 1e-9 {
		t.Fatalf("unexpected rr-adjusted tp: got=%.4f want=101.60", tp)
	}
	if len(reasons) < 2 {
